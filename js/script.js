document.addEventListener('DOMContentLoaded', function () {
    ...
    const burgerMenuCloseBtn = burgerMenu.querySelector('.header-search-btn-close');
    ...
    function getLink(dropdown) {
      const dropdownLiks = dropdown.querySelectorAll('.dropdown-link');
    ...
    function showSerchFormOnTouchscreen() {
      ...
      const logo = siteContainer.querySelector('.logo-link');
      ...
      function showSerchForm() {
        switch (serchFormOpenBtn.dataset.width) {
          case 'large':
           ...
           closeFormBtn.classList.add('serch-form-close-btn');
          ...
          case 'small':
            ...
            logo.classList.add('logo-link_unvisible');
        ...
        switch (serchFormOpenBtn.dataset.width) {
          ...
          case 'small':
            ...
            setTimeout(() => {
              ...
              logo.classList.remove('logo-link_unvisible');
    ...
    document.querySelectorAll('.dropdown-simplebar').forEach(el => {23
      new SimpleBar(el)
    });
    ...
    // Модальное окно в Галерее - информация о картинке
    let previousActiveElement = null;
  
    openPictureCardButtons.forEach((openPictureCardButton) => {
      openPictureCardButton.addEventListener('click', (event) => {
        const imgElement = event.currentTarget.querySelector('.gallery-slide-img');
    ...
    // Плавный скролл к секции по нажатию на навигацию верхнего меню
    ...
    // Таб Каталоге, рефреш аккордиона
    $(function () {
      const artistData = {
    ...
    const mySwiperEvents = new Swiper(eventsSlider, {
      slidesPerView: 1,
